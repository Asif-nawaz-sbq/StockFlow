 ...
 *
 * Production sizing differs from staging in three places: Multi-AZ on the
 * database, a Redis replica with automatic failover, and a scaling floor of
 * two tasks per service so a single task failure is not an outage.
 *
